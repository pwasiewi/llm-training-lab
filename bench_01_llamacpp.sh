#!/bin/bash
# bench_01_llamacpp.sh — benchmark mainline llama.cpp na modelu GGUF (MoE offload)
...
# 10 OOMs on first decode at 32K since 2026-07-13 (build 9999); 12 verified.
SERVER_NCMOE="${SERVER_NCMOE:-12}"
THREADS="${THREADS:-16}"
CTX="${CTX:-32768}"
PORT="${PORT:-8090}"
...
# GPU benchmark needs a mostly free GPU — a forgotten llama-server (aillama)
# holding VRAM shifts the OOM floor and fakes the results.
vram_check() {
	local used
	used=$(nvidia-smi --query-gpu=memory.used --format=csv,noheader,nounits 2>/dev/null | head -1) || return 0
	if [[ -n "$used" && "$used" -gt 3072 ]]; then
		echo "UWAGA: GPU ma już zajęte ${used} MiB VRAM (llama-server w tle? → aillama stop) — OOM/wyniki będą zakłamane." >&2
	fi
}
...
Benchmark mainline llama.cpp: sweep llama-bench (pp512/tg128) po wartościach
--n-cpu-moe oraz realne zapytanie do llama-server z pomiarem timings.

Opcje:
  -b    tylko sweep llama-bench
