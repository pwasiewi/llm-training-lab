...
best_model_path = "outputs/best_hybrid_double_light_res4.pt"
...
class HybridLSTMCNN(nn.Module):
    def __init__(self, config):
        ...
        self.lstm2 = nn.LSTM(config.hidden_dim * 2,
                             config.hidden_dim,
                             bidirectional=True,
                             batch_first=True)

        self.lstm3 = nn.LSTM(config.hidden_dim * 2,
                             config.hidden_dim,
                             bidirectional=True,
                             batch_first=True)
    ...
    def forward(self, x):
        ...
        lstm2_out = lstm2_out + lstm1_out  # Skip connection

        # Third LSTM with residual connection
        lstm3_out, _ = self.lstm3(lstm2_out)
        lstm3_out = lstm3_out + lstm2_out  # Skip connection

        # CNN processing
        lstm_out = lstm3_out.unsqueeze(1)
...
def main():
    ...
    tokenizer = create_tokenizer(train_texts+test_texts)
