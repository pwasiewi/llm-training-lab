# Merge a grpo_07_2 LoRA checkpoint into a standalone 16-bit model that vLLM (_test.py) can load.
# CPU-only on purpose: unsloth's in-training save_pretrained_merged(fast_inference=True)
# corrupted the non-fused LoRA modules (o_proj/down_proj hit ~1e13 — garbage reads from
# the live vLLM memory pool, bug (c) aliasing family, 2026-07-24). A PEFT merge from the
# checkpoint touches neither the GPU nor vLLM, so it cannot alias anything.
# Usage: python grpo_07_2_merge.py [checkpoint-935]   (default: newest checkpoint)
...
base_name  = "microsoft/Phi-4-mini-instruct"
model_path = "outputs/lora-grpo-phi4-mini-v2"          # merged output dir (what _test.py reads)
