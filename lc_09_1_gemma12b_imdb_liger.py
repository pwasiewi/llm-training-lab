...
model_path = "outputs/lora-gemma3-imdb-liger"

# Load pre-trained model and tokenizer
model_name = "google/gemma-3-12b-pt"

# Liger variant of lc_09: patch the Gemma-3 TEXT-stack module classes with
# fused Triton kernels (RMSNorm, RoPE, GeGLU) BEFORE from_pretrained so the
# model is built on the patched classes. gemma-3-12b-pt is the multimodal
# config; the text tower (Gemma3TextModel) is what SEQ_CLS exercises, so the
# _text patcher covers it and leaves the vision tower alone.
# FusedLinearCrossEntropy targets the CausalLM head only, so it is inert for
# SEQ_CLS. Compare against lc_09_gemma12b_imdb.py.
...
print("Liger kernels applied to Gemma-3 text classes")
